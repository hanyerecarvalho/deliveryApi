 ...
 */
class ValidateRestaurantesId implements MiddlewareInterface
{
    /**
    ...
    public function process(Request $request, RequestHandler $handler): Response
    {
        ...
        // -----------------------------------------------------------
        if (!isset($routeArgs['idRestaurante']) || $routeArgs['idRestaurante'] === "") {
            throw new ErrorResponse(
                httpCode: 400,
                message: "Erro na validação de dados",
                error: [
                    "message" => "O parâmetro 'idRestaurante' é obrigatório!"  // Mensagem detalhada
                ]
            );
        }
